...
class PupyConfig(RawConfigParser):
    ...
    def __init__(self, config='pupy.conf'):
        ...
        # Config và dữ liệu trong repo, không dùng ~/.pupy
        self.default_file = path.join(self.root, "conf", config)
        self.user_root = self.root
        self.user_path = self.default_file

        self.files = [self.default_file]
        cwd_conf = path.join(os.getcwd(), config)
        if path.isfile(cwd_conf):
            self.files.append(cwd_conf)
        self.randoms = {}
        self.command_line = {}
    ...
    def get_path(self, filepath, substitutions={}, create=True, dir=False):
        ...
        if dir and path.isdir(retfilepath):
            ...
            if dir:
                ...
                # Nếu không có thư mục (file ngay tại CWD, ví dụ ".pupy_history")
                # thì không cần tạo dir (tránh makedirs('') gây WinError trên Windows)
                if dirpath and not path.isdir(dirpath):
                    makedirs(dirpath)
