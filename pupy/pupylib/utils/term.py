...
class AnyIOWrapper(object):
    ...
    def __init__(self, pyfile):
        ...
        # Python 3: TextIOWrapper.buffer là stream nhị phân gốc — dùng trực tiếp
        # để tránh ValueError trên Windows (os.fdopen với 'ab' không tương thích).
        if hasattr(pyfile, 'buffer'):
            self.fileobj = pyfile.buffer
        else:
            self.fileobj = os.fdopen(pyfile.fileno(), 'ab', 0)
...
def to_bytes(value):
    if isinstance(value, bytes):
        ...
        return str(value).encode(DEFAULT_MULTIBYTE_CP)
