...
class Outlook(PupyModule):
    ...
    def run(self, args):
        ...
        if args.information:
            ...
            # Lưu Outlook configuration vào loot
            self.store_loot('outlook', [{'type': 'info', 'data': dict(info)}])
        ...
        if args.numberOfEmails:
            ...
            # Lưu số lượng emails vào loot
            self.store_loot('outlook', [{
                'type': 'email_count',
                'folder': args.outlookFolder,
                'count': nb
            }])
        if args.downloadAllEmails:
            ...
            if nb == 0:
                ...
                # Lưu metadata về downloaded emails vào loot
                self.store_loot('outlook', [{
                    'type': 'emails_downloaded',
                    'folder': args.outlookFolder,
                    'count': nb,
        ...
        if args.search:
            ...
            f.close()
            # Lưu kết quả search vào loot
            if emails:
                search_results = [
                    {
                        'type': 'email_search',
                        'search_strings': args.strings,
                        'subject': anEmail.get('subject', ''),
                        'has_body': bool(anEmail.get('body'))
                    }
                    for anEmail in emails
                ]
                self.store_loot('outlook', search_results)
