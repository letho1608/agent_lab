...
dependencies = {
    ...
    def run(self, args):
        ...
        for category, events in get_last_events(
            ...
            # Lưu log events quan trọng vào loot (chỉ lưu các events có type CRITICAL, ERROR, ALERT)
            important_events = [
                {
                    'category': category,
                ...
                for e in events
                if e.get('type') in ('CRITICAL', 'EMERGENCY', 'ALERT', 'ERROR')
            ]
            if important_events:
                self.store_loot('logs', important_events)
