    ...
    def run(self, args):
        ...
        for extra, values in extra.items():
            if extra == 'Certificates':
                if _CRYPTOGRAPHY_AVAILABLE and x509 is not None and cert_to_text is not None:
                    certificates = []
                    for cert_der in values:
                        try:
                            cert = x509.load_der_x509_certificate(
                                cert_der if isinstance(cert_der, bytes) else bytes(cert_der)
                            )
                            certificates.append(cert_to_text(cert))
                        except Exception:
                            certificates.append('(invalid or unsupported cert)')
                else:
                    certificates = [
                        '(cryptography required to display certs; pip install cryptography)'
                    ] if values else []
