    ...
    def search(self, args):
        ...
        if isinstance(results, dict):
            for realm, results in results.items():
                ...
                # Lưu kết quả AD search vào loot
                if results:
                    loot_entries = [
                        {**dict(result), 'realm': realm}
                        for result in results
                    ]
                    self.store_loot('ad_search', loot_entries)
        else:
            self._output_search_results(results, fields, args.table)
            # Lưu kết quả AD search vào loot
            if results:
                self.store_loot('ad_search', results)
    ...
    def dump(self, args):
        ...
        def on_data(name, payload):
            if name == 'dns':
                ...
                # Lưu AD dump records vào loot
                if records_list:
                    loot_entries = [
                        {**dict(record), 'dump_name': name}
                        for record in records_list
                    ]
                    self.store_loot('ad_dump', loot_entries)
