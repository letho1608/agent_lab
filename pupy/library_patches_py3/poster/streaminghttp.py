"""Streaming HTTP uploads module (Python 3).

Extends http.client and urllib.request so that iterable objects
can be used in the body of HTTP requests.

Call :func:`register_openers()` to register the streaming handlers,
then use iterable objects in the body of HTTP requests.

**N.B.** You must specify a Content-Length header if using an iterable.
...
class _StreamingHTTPMixin:
    ...
    def send(self, value):
        ...
        method.
        """
        # Based on httplib.HTTPConnection.send()
...
class StreamingHTTPRedirectHandler(HTTPRedirectHandler):
    """Subclass of HTTPRedirectHandler that overrides redirect_request
    to handle redirected POST requests (strips Content-Type/Content-Length)."""

    handler_order = HTTPRedirectHandler.handler_order - 1

    # redirect_request implementation
...
class StreamingHTTPHandler(HTTPHandler):
    """Subclass of HTTPHandler that uses StreamingHTTPConnection."""
    ...
    def http_request(self, req):
        ...
        if req.data is not None:
            data = req.data
            if hasattr(data, 'read') or hasattr(data, '__next__') or hasattr(data, 'next'):
...
class StreamingHTTPSHandler(HTTPSHandler):
    """Subclass of HTTPSHandler that uses StreamingHTTPSConnection."""
...
def register_openers():
    """Register the streaming http handlers in the global default opener.
