    ...
    if args.module:
        ...
        for module in server.iter_modules():
            if module.get_name().lower() == args.module.lower():
                _show_module_help(module)
                break
        else:
            # Khi không có client, iter_modules() rỗng; thử resolve module theo tên từ server.modules
            if getattr(server, 'modules', None) and args.module in server.modules:
                from inspect import isclass
                from pupy.pupylib.PupyModule import PupyModule
                modobj = server.modules[args.module]
                for item_name in dir(modobj):
                    ...
                    if isclass(item) and issubclass(item, PupyModule) and item != PupyModule:
                        if item.get_name().lower() == args.module.lower():
                            _show_module_help(item)
                            break
        ...
        if args.modules:
            ...
            # Khi không có client, iter_modules() trả về rỗng; lấy danh sách từ server.modules
            if not modules and getattr(server, 'modules', None):
                from inspect import isclass
                from pupy.pupylib.PupyModule import PupyModule
                for _name, modobj in server.modules.items():
                    for item_name in dir(modobj):
                        item = getattr(modobj, item_name)
                        if isclass(item) and issubclass(item, PupyModule) and item != PupyModule:
                            modules.append(item)
                            break
            modules = sorted(modules, key=(lambda x: getattr(x, 'category', 'general')))
            table = []
