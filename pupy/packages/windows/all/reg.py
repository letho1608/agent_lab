...
def EnumKey(key, index):
    ...
    raise_on_error(rc)
    return wstring_at(tmpbuf, length.value).rstrip('\x00')
...
def _search(
    ...
    if regex:
        ...
        if equals:
            compare = lambda x: \
                term_re.match(x) if isinstance(x, str) else False
        else:
            compare = lambda x: \
                term_re.search(x) if isinstance(x, str) else False
...
def enum(path=None):
    ...
    try:
        ...
        for item in Key(path):
            if type(item) == Key:
                tupleized.append((True, str(item)))
            else:
                tupleized.append((
