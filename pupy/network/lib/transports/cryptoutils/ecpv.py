...
from tinyec.registry import get_curve as _get_curve
from tinyec.ec import Point, Inf

def get_curve(name):
    curve = _get_curve(name)
    if not hasattr(curve, 'bytes'):
        curve.bytes = (curve.field.bit_length() + 7) // 8
    return curve
...
def ec2osp(point):
    ...
    # Check if point is Inf (some versions might use different comparison)
    if not hasattr(point, 'x') or point.x is None:
        return b'\x00'

    coord_len = point.curve.bytes
    x_bytes = to_bytes(point.x, coord_len)
    y_bytes = to_bytes(point.y, coord_len)
    return b'\x04' + x_bytes + y_bytes

def osp2ec(curve, data):
    if not data:
        return None
    if data[0] == 0x00:
        return Inf
    if data[0] != 0x04:
        return None
    coord_len = curve.bytes
    if len(data) != 1 + 2 * coord_len:
        return None
    x = from_bytes(data[1:1+coord_len])
    y = from_bytes(data[1+coord_len:1+2*coord_len])
