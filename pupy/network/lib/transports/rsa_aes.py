...
""" RSA+AES transport: cryptography (PKCS1v15). """
...
class RSA_AESClient(RSA_AESTransport):
    ...
    def on_connect(self):
        pk = load_pem_public_key(_ensure_bytes(self._pubkey))
        self.aes_key = get_random(self.key_size)
        self.enc_cipher = NewAESCipher(self.aes_key, self._iv_enc)

        pkey = pk.encrypt(self.aes_key, padding.PKCS1v15())
...
class RSA_AESServer(RSA_AESTransport):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.pk = load_pem_private_key(_ensure_bytes(self._privkey), password=None)
    ...
    def downstream_recv(self, data):
        try:
            if self.aes_key is None:  # receive aes key
                ...
                try:
                    self.aes_key = self.pk.decrypt(cmsg, padding.PKCS1v15())
                except ValueError:
